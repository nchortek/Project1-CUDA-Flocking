...
void mainLoop() {
    ...
    const int targetFrames = 1000;
    const double notMeasuring = -1.0;

    double fps = 0;
    int framesSinceLastMeasurement = 0;
...
#endif
    {
        ...
        // Check if we should begin average FPS measurement
        if (measureStart == notMeasuring && curTime - startTime >= warmupSeconds)
        {
            measureStart = curTime;
        }

        // If measurement has started
        if (measureStart != notMeasuring)
        {
            measuredFrames++;
        }

        if (double timeSinceLastMeasurement = curTime - timebase; timeSinceLastMeasurement > 1.0)
        {
            fps = framesSinceLastMeasurement / timeSinceLastMeasurement;
            timebase = curTime;
    ...
    double totalTime = glfwGetTime() - measureStart;
    if (measuredFrames > 0 && totalTime > 0.0)
    {
        double averageFps = measuredFrames / totalTime;
        std::cout << "Average FPS: " << averageFps
            << " (" << measuredFrames << " frames over "
